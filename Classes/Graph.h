...
vector<T> Graph<T>::topsort() const {
    vector<T> res;
    vector<Vertex<T>*> L;  // List to store vertices in topological order
    queue<Vertex<T>*> q;   // Queue for BFS
    ...
    // Check if the graph has a cycle (some vertices are not visited)
    if (L.size() != vertexSet.size()) {
        // Graph has a cycle
        return res;  // Empty vector indicating failure
    }

    // Populate the result vector with the topological order
    for (auto v : L) {
